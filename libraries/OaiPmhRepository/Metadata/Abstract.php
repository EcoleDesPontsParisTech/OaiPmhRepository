...
abstract class OaiPmhRepository_Metadata_Abstract extends OaiPmhRepository_OaiXmlGeneratorAbstract
{
    ...
    public function appendHeader($parentElement)
    {
        ...
        // Insert setSpec for collections
        $collection = $this->item->Collection;
        if ($collection) {
            $identifier = metadata($collection, array("Dublin Core", "Identifier"));
            if ($identifier) {
                $headerData['setSpec'] = OaiPmhRepository_ResponseGenerator::INSTITUTION_PREFIX_SPEC.$identifier;
                $elem = $this->createElementWithChildren($parentElement, 'header', $headerData);

                // Insert setSpec for document types
                $type = metadata($this->item, array("Dublin Core", "Type"));

                $type = strtolower($type);

                if (count($type) > 0) {
                    $infos  = OaiPmhRepository_ResponseGenerator::getSetInfosFromValue($type);
                    $documentTypeSpec = $this->document->createElement('setSpec', $infos['spec']);
                    $elem->appendChild($documentTypeSpec);
                }
            }
        }
