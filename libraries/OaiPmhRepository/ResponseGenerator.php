...
{
    /**
     * Define various constants used for Harvesting by Gallica & Europeana
     * Static vars are used for arrays because PHP < 5.6 doesn't support arrays as constants
     * @author Franck Dupont <technique@limonadeandco.fr>
     */

    // Prefix for the <setSpec> tag
    const INSTITUTION_PREFIX_SPEC = 'institution_';

    // Prefix for the <setName> tag
    const INSTITUTION_PREFIX_NAME = 'INSTITUTION : ';

    // Set name of the printed serial ("périodiques")
    const INSTITUTION_PRINTER_SERIAL_SPEC = 'institution_periodiques';

    // Ids of collection exposed in the XML
    public static $EXPOSED_COLLECTIONS_IDS = array(1,2,3,10,11,12);

    // Values of the <setSpec>, <setName> and database for each document type exposed in the XML
    public static $EXPOSED_DOCUMENTS_TYPES = array(
            array(  "spec"  => "institution_imprimes",
                    "name"  => "Imprimés",
                    "value" => "monographie imprimée"),
            array(  "spec"  => "institution_manuscrits",
                    "name"  => "Manuscrits",
                    "value" => "manuscrit"),
            array(  "spec"  => "institution_cartes",
                    "name"  => "Cartes",
                    "value" => "carte"),
            array(  "spec"  => "institution_periodiques",
                    "name"  => "Périodiques",
                    "value" => "publication en série imprimée"),
            array(  "spec"  => "institution_photographies",
                    "name"  => "Photographies",
                    "value" => "photographie"),
            array(  "spec"  => "institution_dessins",
                    "name"  => "Dessins",
                    "value" => "dessin")
        );
    ...
    public function __construct($query)
    {
        ...
        $xslt = $this->document->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="'.WEB_PUBLIC_THEME.'/YOURTHEME/oai-pmh/xsl/oai-pmh-repository.xsl"');
    ...
    public function identify()
    {
        ...
        $elements = array(
             ...
             *  Document le plus ancien (date d'ajout dans le dépôt : 14/02/2013, unix=1360861200)
             **/
            'earliestDatestamp' => OaiPmhRepository_Date::unixToUtc(1360861200),
            'deletedRecord'     => 'no',
    ...
    private function listSets()
    {
        ...
        if(!$this->error) {
            ...
            foreach ($collections as $collection) {
                ...
                if ($this->_collectionIsExposed($collection) ) { // This collection is exposed

                    $dc_identifier = metadata($collection,array('Dublin Core','Identifier'));
                	$dc_title = metadata($collection,array('Dublin Core','Title'));

                    $elements = array( 'setSpec' => self::INSTITUTION_PREFIX_SPEC . $dc_identifier,
                                       'setName' => self::INSTITUTION_PREFIX_NAME . $dc_title);
        ...
        // Expose sets by documents type

        foreach(self::$EXPOSED_DOCUMENTS_TYPES as $type) {

            $setName = $type['name'];
            if (strpos($setName, self::INSTITUTION_PREFIX_NAME)>=0)
                $setName = self::INSTITUTION_PREFIX_NAME . $type['name'];

            $elements = array( 'setSpec' => $type['spec'],
                               'setName' => $setName);
    ...
    private function listResponse($verb, $metadataPrefix, $cursor, $set, $from, $until) {
        ...
        if($set) {
            if ($set == self::INSTITUTION_PRINTER_SERIAL_SPEC) {
                $itemTable->filterByPrintedSerial($select);
            }
            elseif ($infos = $this->_isSetByDocumentType($set)) {
                $itemTable->filterByDcType($select, $infos['value']);
            } else {
                $set = str_replace(self::INSTITUTION_PREFIX_SPEC, '', $this->_getParam('set'));
                $itemTable->filterByCollectionDcIdentifier($select, $set);
            }
        }
        if($from) {
        ...
        $in = '('.implode(',', self::$EXPOSED_COLLECTIONS_IDS).')';
        $select->where("$alias.collection_id IN ".$in);
    ...
    /**
     * Returns if whether or not a collection is exposed in the XML
     *
     * @author Franck Dupont <technique@limonadeandco.fr>
     * @param Collection|integer $collection Either a Collection object or the collection ID
     * @return bool
     */
    private function _collectionIsExposed($collection)
    {
        $id = $collection instanceof Collection ? $collection->id : $collection;

        return in_array($id, self::$EXPOSED_COLLECTIONS_IDS);
    }

    /**
     * Returns infos about a Set if the set required is a set by document type, otherwise FALSE
     * Infos are extract from self::$EXPOSED_DOCUMENTS_TYPES constant
     *
     * @author Franck Dupont <technique@limonadeandco.fr>
     * @param string $set_name The name of the set
     * @return array|false
     */
    private function _isSetByDocumentType($set_name)
    {
        foreach (self::$EXPOSED_DOCUMENTS_TYPES as $type) {
            if ($type['spec'] == $set_name)
                return $type;
        }
        return false;
    }

    public static function getSetInfosFromValue($set_value)
    {
        foreach (self::$EXPOSED_DOCUMENTS_TYPES as $type) {
            if ( $type['value'] == $set_value)
                return $type;
        }
    }
}
